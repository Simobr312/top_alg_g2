...
\begin{lemma} [Sollevamento di un cammino di $\Sph{1}$ in $\R$] \nl
    \begin{enumerate}
        ...
        \] 
    \end{enumerate}
    La dimostrazione di questo lemma sar\`a discussa in un contesto pi\`u generale nella sezione sui rivestimenti.
\end{lemma}
...
\begin{definition} [Retrazione per deformazione] \nl
...
\begin{proposition} [I rivestimenti sono omeomorfismi locali] \nl
...
\begin{theorem} [Fibre di un rivestimento] \nl
...
\begin{corollary} [I rivestimenti sono surgettivi]
...
\begin{proposition} [Unicit\`a di sollevamenti di mappe continue qualunque] \nl
...
\begin{proof} \nl
    ...
    % https://q.uiver.app/#q=WzAsMyxbMCwwLCJcXHRpbGRle1hfMX0iXSxbMiwwLCJcXHRpbGRle1hfMn0iXSxbMSwxLCJYIl0sWzAsMiwiXFxwaV8xIiwyXSxbMSwyLCJcXHBpXzIiXSxbMCwxLCJcXHZhcnBoaSIsMix7Im9mZnNldCI6MX1dLFsxLDAsIlxccHNpIiwyLHsib2Zmc2V0IjoxfV1d
    \[\begin{tikzcd}
	{\tilde{X_1}} && {\tilde{X_2}} \\
	& X
...
\end{tikzcd}\]
    Cio\`e
    % https://q.uiver.app/#q=WzAsMyxbMCwwLCJcXHRpbGRle1hfMX0iXSxbMiwwLCJcXHRpbGRle1hfMX0iXSxbMSwxLCJYIl0sWzAsMiwiXFxwaV8xIiwyXSxbMSwyLCJcXHBpXzEiXSxbMCwxLCJcXHBzaSBcXGNpcmMgXFx2YXJwaGkiLDIseyJvZmZzZXQiOjF9XSxbMCwxLCJcXGlkX3tcXHRpbGRle1hfMX19IiwwLHsib2Zmc2V0IjotMX1dXQ==
    \[\begin{tikzcd}
	{\tilde{X_1}} && {\tilde{X_1}} \\
	& X
...
\begin{proof} \nl
    Fissato un punto $x_0 \in X$, il rivestimento universale $\tilde{X}$ \`e definito come
    \[
        \tilde{X} := \set{\text{cammini } f: I \to X \mid f(0) = x_0} / \sim
    \]
    con $\pi: \tilde{X} \to X$ definita come
    \[
        \pi([f]) := f(1).
    \]
    \begin{itemize}
        \item La topologia di $\tilde{X}$ ha come base la seguente: se $\tilde{x} \in \Tilde{X}$ e $U$ intorno semplicemente connesso di $f(1) = \pi(\tilde{x}) \in X$(che esiste per le ipotesi),
        \[
            \tilde{U_{\tilde{x}}} := \set{ [f * g] \mid g: I \to U \text{ cammino tale che } g(0) = f(1)}/~.
        \]
        dove $~$ \`e l'omotopia di cammini ad estremi fissi. \nl
        Dato che $U$ \`e semplicemente connesso, le classi di omotopia dei cammini $g$ dipendono solo dal punto finale $g(1)$. \nl
        Resta da mostrare che gli $\tilde{U_{\tilde{x}}}$ sono una base di una topologia su $\tilde{X}$ e che $\pi$ \`e continua. \nl
    \item $\pi$ \`e un rivestimento e gli intorni ben rivestiti sono proprio quelli semplicemente connessi dati dalle ipotesi,
    infatti $\forall x \in X$ se $U$ \`e un intorno semplicemente connesso di $x$, allora
    \[
        \pi^{-1}(U) = \bigsqcup_{\tilde{x} \in U} \tilde{U_{\tilde{x}}}
    \]
    \item $\tilde{X}$ \`e connesso per archi, infatti se $\tilde{x_0} = [I \to \set{x_0}] \in \tilde{X}$ \`e la classe del cammino costante
    e $\tilde{x} = [f] \in \tilde{X}$ un qualsiasi altro elemento, allora il cammino
    \[
        \tilde{f}: I \to \tilde{X}: s \mapsto \left[t \mapsto f(st)\right]
    \]
    parte da $\tilde{f}(0) = [t \mapsto f(0)] = \tilde{x_0}$ e arriva a $\tilde{f}(1) = [t \mapsto f(t)] = \tilde{x}$, quindi $\tilde{X}$ \`e connesso per archi. \nl
    Notare che tale cammino, essendo un sollevamento \`e unico. \nl
    \item $\tilde{X}$ \`e semplicemente connesso, infatti se $\tilde{\gamma}: I \to \tilde{X}$ \`e un cammino chiuso tale che $\tilde{x} = \tilde{\gamma}(0) = \tilde{\gamma}(1)$,
    allora $\gamma := \pi \circ \tilde{\gamma}: I \to X$ \`e tale che $\gamma(0) = \gamma(1) = x_0$, ma $\gamma$ \`e omotopo al cammino
    costante $I \to \set{x_0}$, e per unicit\`a del sollevamento di cammini si ha che
    \[
        \tilde{\gamma} \sim I \to \set{\tilde{x}}
    \]
    che \`e l'unico sollevametno del cammino costatne $I \to \set{x_0}$.
    \end{itemize}    
\end{proof}

\subsection{Dimostrazione del teorema di Seifert-Van Kampen}

Si consdierino le seguenti due costruzinoi:
\begin{enumerate}
    \item Ad un omomorfismo surgettivo corrisponde un rivestimento di Galois: \nl
        Sia $\varrho: \pi_1(X, x_0) \onto G$ un omomorfismo surgettivo e sia $N := \ker(\varrho)$ il suo nucleo. \nl
        $N$ agisce su $\tilde{X}$, tramite l'azione $g.\tilde{x} := tilde{x} * g$, tale azione si verifica
        essere propriamente discontinua e quindi definisce un riestimento connesso
        \[
            p_\varrho: Y_\varrho := \tilde{X}/N \to X
        \]
        Tale rivestimento risulta inoltre essere di Galois, $\pi^{-1}(U) \cong U \times \pi_1(X, x_0)/N$ per ogni intorno ben rivestito $U$ di $x_0$, dove $\pi_1(X, x_0)/N$ \`e munito dela topologia discreta. \nl
        Segue che
        \[
            p^{-1}(U) \cong U \times \left(\frac{\pi_1(X, x_0)}{N}\right) \cong U \times G.
        \]
    \item Dato un rivestimento $p: Y \to X$ di Galois, si definisce l'omomorfismo
        \[
            \varrho: \pi_1(X, x_0) \to \Aut{Y/X} =: G
        \]
        come segue: fissato un $\tilde{x} \in p^{-1}(x_0)$, sia $g = [\gamma] \in \pi_1(X, x_0)$ e $\tilde{x_g} := \tilde{\gamma}(1) \in p^{-1}(x_0)$, dove
        Fissato un $y \in p^{-1}(x_0)$, per un teorema precedente esiste un unico morfismo di rivestimenti $\varphi: \tilde{X} \to Y$ tale che $\varphi(x_0) = y$. \nl
        Sia ora $\psi \in \Aut{Y/X}$ e $y_\psi := (\varphi \circ \psi)(\tilde{x_0}) \in p^{-1}(x_0)$. \nl
        Poich\'e $p$ \`e di Galois, esiste un unico $g \in G = \Aut{Y/X}$ tale che $g(y) = y_\psi$. Vale dunque che
        \[
            (\varphi \circ \psi)(\tilde{x_0}) = (g \circ \varphi)(\tilde(x_0)) 
        \] 
        e 
        \[
            p \circ \varphi \circ \psi = p \circ g \circ \psi
        \]
        cio\`e il seguente diagramma commuta:
        % https://q.uiver.app/#q=
        \[\begin{tikzcd}
            {\tilde{X}} & {\tilde{X}} \\
            Y & Y \\
            X & X
        ...
        \end{tikzcd}\]
        A questo punto si definisce l'ommorfismo
        \[
            \varrho: \pi_1(X, x_0) \to \Aut{Y/X} = G: \psi \mapsto g_\psi
        \]
        che risulta essere omomorfismo perch\'e $g_2g_1$ \`e l'unico morfismo che fa commutare il seguente diagramma:
       % https://q.uiver.app/#q=
        \[\begin{tikzcd}
            {\tilde{X}} & {\tilde{X}} & {\tilde{X}} \\
            Y & Y & Y \\
            X & X & X
            \arrow["{\psi_1}", from=1-1, to=1-2]
            \arrow["\varphi", from=1-1, to=2-1]
            \arrow["{\psi_2}", from=1-2, to=1-3]
            \arrow["\varphi", from=1-2, to=2-2]
            \arrow["\varphi", from=1-3, to=2-3]
            \arrow["{g_1}", dashed, from=2-1, to=2-2]
            \arrow["{g_2g_1}"{description}, curve={height=6pt}, dashed, from=2-1, to=2-3]
            \arrow["p", from=2-1, to=3-1]
            \arrow["{g_2}", dashed, from=2-2, to=2-3]
            \arrow["p", from=2-2, to=3-2]
            \arrow["p", from=2-3, to=3-3]
            \arrow["\id"', from=3-1, to=3-2]
            \arrow["\id"', from=3-2, to=3-3]
        \end{tikzcd}\]
        Inoltre $\varrho$ \`e surgettivo,
\end{enumerate}

\begin{theorem} \nl
    Le due costruzioni precedente sono l'una l'inversa dell'altra ed inducono una corrispondenza :
    \begin{tikzcd}
        {\set{\varrho: \pi_1(X, x_0) \to G}} \arrow[r,leftrightarrow] & {\set{\text{rivestimenti di Galois } p: Y \to X \mid y \in p^{-1}(x_0) \text{ fisso}}}
    \end{tikzcd}
\end{theorem}

\begin{proof} [del Teorema di Seifert-Van-Kampen (preliminare)] \nl
    Sia $X = X_1 \cup X_2$ la unione di due aperti $X_1$ e $X_2$ connessi per archi, tali che $X_1 \cap X_2$ sia connesso per archi.
    Per la corrispondenza enunciata prima la situazione del teorema \`e la seguente:
    % https://q.uiver.app/#q=
    \[\begin{tikzcd}
	& {\pi_1\left(X_1, x_0\right)} \\
	{\pi_1\left(X_1 \cap X_2, x_0\right)} && {\pi_1\left(X, x_0\right)} & G \\
	& {\pi_1\left(X_2, x_0\right)}
	\arrow["{i_{1*}}"', from=1-2, to=2-3]
	\arrow["{\varrho_1}", from=1-2, to=2-4]
	\arrow["{j_{1*}}", from=2-1, to=1-2]
	\arrow["{j_{2*}}"', from=2-1, to=3-2]
	\arrow["{i_{2*}}", from=3-2, to=2-3]
	\arrow["{\varrho_2}"', from=3-2, to=2-4]
    \end{tikzcd}\]
    Si suppone per la dimostrazione che:
    \begin{enumerate}
        \item X \`e localmente semplicemente connesso, dunque esiste il suo rivestimento universale. \nl
        \item (per ora) le mappe $\varrho_1$ e $\varrho_2$ sono surgettive.
    \end{enumerate}
    Per il teorema precedente si ha che esistono due rivestimenti $p_1: Y_1 \to X_1, p_2: Y_2 \to X_2$ che corrispondono rispettivamente
    a $\varrho_1$ e $\varrho_2$ e che hanno come gruppo di Galois proprio $G$.
    Inoltre le restrizioni
    \[
        \restrict{p_i}{p_i^{-1}(X_1 \cap X_2)}: p_i^{-1}(X_1 \cap X_2) \toiso X_1 \cap X_2
    \]
    sono isomorfismi gli unici isomorfismi fissato $x_0 \in X_1 \cap X_2$ e $y_0$ nella preimmagine. \nl
    Per continuare la dimostrazione ci sar\`a bisogno del lemma che segue.
\end{proof}
...
\begin{proof} [del lemma]
    Sia $Y := \frac{Y_1 \sqcup Y_2}{/sim}$ dove $y_1 \sim y_2$ se e solo se $y_i \in p^{-1}(X_1 \cap X_2)$ e $\varphi(y_1) = y_2$. \nl
    Sia $p: Y \to X$ la mappa indotta naturalmente da $p_1$ e $p_2$, cio\`e
    \[
        p(y) := 
        \begin{cases}
            p_1(y) & \text{se } y \in Y_1 \\
            p_2(y) & \text{se } y \in Y_2
        \end{cases}
    \]
    e poich\'e $\restrict{p}{p_i^{-1}(X_i)} \cong p_i$ , si ha che $p$ \`e un rivestimento. \nl 
    Inoltre se $Y_1, Y_2$ sono connessi lo \`e anche $Y$. \nl
    Inoltre per qualche motivo a me non ancora ovvio, se $p_1, p_2$ sono di Galois con gruppo $G$, allora anche $p$ \`e di Galois con gruppo $G$. \nl
\end{proof}

\begin{proof} [del Teorema di Seifert-Van-Kamper (conclusione)] \nl
    La $p$ che esiste per il lemma precedente e che fa commutare il diagramma, e corrisponde proprio ad una 
    $\varphi: \pi_1(X, x_0) \to G$ tale che il diagramma del prodotto amalgamato sia soddisfatto. \nl
    Per dimostrare che il teorema vale anche quando i $\varrho_i$ non sono surgettivi va modificata la costruzione 1 in modo da fare una corrispondenza tra tutti
    gli omomorifsmi e una nuova classe di rivestimenti, detti $G$-rivestimenti. Gli ultimi fatti del corso, puntano a costruire tale corrispondenza.
\end{proof}
...
\begin{enumerate}
    \item Data una funzione $f: \C \to \C$ olomorfa tale che $\abs{f(z)} \leq c_1\abs{z} + c_2$ con $c_1,c_2 \in \R$ costanti.
        ...
        \begin{answer}
            I polinomi di grado minore o uguale a $d$. Dimostrazione molto simile al Teorema di Louville.
        \end{answer}
    \item \`E vero che $f$ come sopra(quindi un polinomio) definisce un rivestimento $\C \to \C$? Per esempio $z \mapsto z^n$ \`e un rivestimento da $\C \to \C$? (Tamas)
        \begin{answer}
            La mappa $\varrho: \C \to \C: z \mapsto z^n$ non può dare un rivestimento perch\'e per esempio, $\varrho^{-1}(0) = \set{0}$, mentre
            $\varrho^{-1}(1) = \set{\zeta_n^i \mid i = 0, \dots ,n-1}$ e quindi non sarebbe ben definito il suo grado. (In generale tale osservazione si pu\`o fare per un poliniomio
            generico al posto di $z^n$ ed usando il teorema fondamentale dell'algebra) \nl
            Tuttavia la mappa $\varrho: \C \setminus \set{0} \to \C \setminus \set{0}: z \mapsto z^n$ da un rivestimento
            indotto dall'azione propriamente discontinua di $\Z / n\Z$ su $\C^*$ data da $[m].z \to (z) \zeta_n^m$, infatti in tal caso le orbite
            dell'azione sono proprio le radici $n$-esime di $z^n$ e quindi le fibre del rivestimento $\varrho: \C \setminus \set{0} \to \C \setminus \set{0}$. \nl
        \end{answer}
\end{enumerate}
...
\begin{enumerate}
    ...
    \end{proof}
    \item Costruzione del cono toologico $\frac{X \times [0,1]}{X \times \set{0}}$. Calcolo del grupo fondamentale per
...
\section{Domande orali del 9 Giugno}

\subsection{Orale}
\begin{enumerate}
    \item Qual \`e la definizione di componente connessa? Si dimostri la buona definizione.
    \item Condizione sufficienti affinch\'e le componenti connesse siano aperte.
    \begin{answer}
        Se lo spazio \`e localmente connesso, allora le componenti connesse sono aperte e chiuse.
    \end{answer}
    \item Qual è la definizione di omeomorfismo locale? Si estenda la nozione di sollevamento di cammino ad un qualunque
    omeomorismso locale \textbf{surgettivo}, il sollevamento esiste? è unico? (TAMAS)
    \begin{answer}
        Tamas è convinto dell'unicit\`a, Frigerio no. \nl
        Alla fine Frigerio convince Tamas che non \`e vero in generale e che per esempio lui sa dimostrarlo $\varphi: Y \to X$
         sia un omeomorifmso locale con $Y$ \textbf{T2}. In generale per\`o non si ha ne esistenza ne unicit\`a. \nl
         In particolare nella dimostrazione si usa il fatto che se $Y$ \`e T2 i limiti di successioni sono unici.
    \end{answer}
\end{enumerate}

\subsection{Orale}
\begin{enumerate}
    \item Come si definisce la restrizione di un rivestimento? (TAMAS)
    \item Sia $X = A_1 \cup A_2$ aperti con $A_1 \cap A_2 \neq \emptyset$ e due rivestimenti
    $p_1: X_1 \to A_1 e p_2: X_2 \to A_2$ tali che $\restrict{p_1}{p_1^{-1}(A_1 \cap A_2)}$ e $\restrict{p_2}{p_2^{-1}(A_1 \cap A_2)}$ sono isomorfismi. \nl
    Come si fa a costruire un rivestimento $p: Y \to X$ come "rincollamento" dei due rivestimenti. (TAMAS)
    \item Scelga una dimostrazione del teorema fondamentale dell'algebra e la faccia.
    \begin{answer}
        Quella con Louville.
    \end{answer}
    \item Come si dimostra il teorema di Louville?
\end{enumerate}


\subsection{Orale}
\begin{enumerate}
    \item Sia $f: \C \to \C$ olomorfa e bigettiva, si trovi una caratterizzazione di $f$.
    \begin{answer}
        Le affinit\`a di $\C$.
    \end{answer}
    \item In uno spazio topologico si dice irriducibile se ogni due aperti non vuoti si intersecano non banalmente.
    Mi dia un esempio di spazio connesso ma non irriducibile. Poi uno connesso, ma non T2.
    \begin{answer}
        Beh $\R$ non \`e irriducibile, La retta a cui aggiungo un punto in modo che non venga T2 come visto a negli esercizi.
        Dimostri che tale esempio è connesso.
    \end{answer}
\end{enumerate}

\subsection{Orale}
\begin{enumerate}
    \item Sia $X$ metrico, compatto. A lezione si è dimostrato che ogni ricoprimento aperto ammette un numero di Lebesgue.
    Qual'\`e la definizione di numero di Lebesgue? Si dimostri l'esistenza del numero di Lebesgue.
\end{enumerate}

\subsection{Orale}
\begin{enumerate}
    \item Esempio di spazio topologico che non \`e T2, ma \`e T1. (TAMAS)
        \begin{answer}
            Una risposta corretta \`e qualsiasi spazio topologico con la topologia indiscreta. \nl
            Per\`o hanno dimostrato che basta un qualsiasi spazio topologico infinito con la topologia cofinita.
        \end{answer}
    \item Prodotto di spazi T2 \`e T2, \`e vero per gli spazi T1? E nel caso del prodotto infinito?
        \begin{answer}
            Certo, perch\'e due punti distinti hanno almeno una coordinata distinta. E vale anche infinito.
        \end{answer}
    \item E se invece si considera un quoziente di uno spazio T1, \`e ancora T1?
        \begin{answer}
            $X/~$ \`e T1 se e solo se le classi di equivalenza sono chiuse in $X$. \nl
        \end{answer}
    \item Esempio concreto di spazio T1 e relazione ~ tale che il quoziente non venga T1. (TAMAS)
        \begin{answer}
            Consiglio di Frigerio: trova un gruppo $G$ la cui azione su $X$ da uno spazio con la topologia indiscreta.
            Azione di $\Q$ su $\R$ per traslazione ed \`e indiscreta perch\'e l'unico aperto saturo \`e tutto $|R$.
        \end{answer}
\end{enumerate}
