...
\begin{proof} [del teorema] \nl
    ...
    \end{tikzcd}
    \]
    Cio\`e vale che $ \varphi_* \circ (\tau_f \circ \psi) = \id = (\varphi_* \circ \tau_f) \circ \psi$, quindi \`e un fatto generale che
    segue che $\varphi_*$ \`e surgetiva e $\psi_*$ \`e surgettiva. \nl
    Inoltre facendo lo stesso ragionamento con $\varphi \circ \psi \sim \id_Y$ si ha che 
    $\varphi_*$ \`e surgettiva e $\psi_*$ \`e iniettiva, quindi sono isomorfismi tra i rispettivi
    gruppi fondamentali.   
\end{proof}
...
\begin{theorem} [Gruppo fondamentale del cerchio] \nl
    \[
        \pi_1\left(S^1 \right) \cong \Z.
    \]
    Ed il cammino chiuso $t \mapsto e^{2\pi i t}$ rappresenta il generatore del gruppo fondamentale $\pi_1\left(S^1, 1\right)$.
\end{theorem}

\begin{definition} [Mappa esponenziale] \nl
    Si definisce la mappa
    \[
        \rho: \R \to S^1: t \mapsto e^{2\pi i t}
    \]
\end{definition}

\begin{lemma} [Sollevamento di un cammino di $S^1$ in $\R$] \nl
    \begin{enumerate}
        \item Per ogni cammino chiuso $f: I \to S^1$ con $f(0) = f(1)$, 
...
\begin{proof} (Lemma $\implies$ Teorema) \nl
    ...
    \[
        \Phi: \pi_1\left(S^1, 1\right) \to \Z: [f] \mapsto \tilde{f}(1)
    \]Il terminale mi dice di aver committato, ma su github la repository non sembra essere commi
    ...
    \].
    Si dimostra ora la surgettivit\`a di $\Phi$, infatti dato il cammino chiuso $f_1: I \to S^1: t \mapsto e^{2\pi i t}$, si ha che
    \[
    ...
    se $f: I \to S^1$ \`e un cammino chiuso tale che $\tilde{f}(1) = 0$, si ha che $\tilde{f}$ \`e un cammino chiuso in $\R$ che parte da $0$ e torna a $0$, quindi
...
\begin{proof} (del teorema) \nl
    \begin{enumerate}
        \item si consideri il ricoprimento di aperti di $S^1$ dato da due aperti $U_0, U_1$, archi che si intersecano in due componenti 
        connesse per archi di $S^1$, una che contiene $1$ e l'altra che contiene $-1$. 
        \nl
        DISEGNO DA FARE
        \nl
        \nl
